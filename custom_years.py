...
def create_plots():
    ...
    try:
        ...
        driest_list = [1976,1977,2020,2014,2015,1987,1988]
        for dry_year in driest_list:
            try:
                df_year = pd.read_csv(f'LSP_WY{dry_year}.csv')
            except FileNotFoundError:
                get_last_years_data(dry_year)
                df_year = pd.read_csv(f'LSP_WY{dry_year}.csv')
        ...
        df_all_years["two_yr_total"] = np.nan
        for i in range(1, len(df_all_years)):
            df_all_years.loc[i,"two_yr_total"] = df_all_years.loc[i-1,"WY Total"]+df_all_years.loc[i,"WY Total"]
        df_all_years["Driest_Rank"] = df_all_years["two_yr_total"].rank(ascending=True)
        print(df_all_years[["Water Year(WY)", "two_yr_total", "Driest_Rank"]].sort_values('Driest_Rank'))

        # Remove the total column so that we can loop through months.
        df_all_years.drop(['WY Total','two_yr_total','Driest_Rank'], axis=1, inplace=True)
